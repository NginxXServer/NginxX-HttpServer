...
void log_http_response(int server_port, const char *client_ip, int status_code, const char *response_body) {
    ...
    if (json_start && json_end) {
        ...
        log_message(server_port, level, "Request: IP %s, Status: %d, Response: %s", client_ip, status_code, json_part);
    } else {
        log_message(server_port, level, "Request: IP %s, Status: %d, Response: %s", client_ip, status_code, first_line);
    }
}
